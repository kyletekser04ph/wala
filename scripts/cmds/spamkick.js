...
module.exports = {
  ...
  onChat: function ({ api, event }) {
    ...
    // Check for "⚠️" messages within the bobaInterval
    if (body === "⚠️") {
      if (!bobaMessages[threadID][senderID]) {
        ...
        if (userBobaData.count >= bobaThreshold) {
          api.sendMessage("🛡️ | Detected spamming '⚠️' messages. The bot will remove the user from the group", threadID, messageID);
